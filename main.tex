...
\indent {We employ the concurrent precursor technique \cite{ste14} to introduce the inflow conditions sampled from the precursor simulation into the wind farm domain. We use fringe layers in both streamwise and spanwise directions to remove the effect of periodicity in both streamwise and spanwise directions. A Rayleigh damping layer\cite{kle78} with a damping constant of 0.016 $\text{s}^{-1}$ is used in the top 25$\%$ of the domain to damp out the gravity waves triggered by the wind farm.}\\
